...
def main(args):
    ...
    # Replace the SGD optimizer with AdamW, which is often better for non-stationary losses
    optimizer = AdamW(model.parameters(),
                            lr=args.lr,
                            weight_decay=1e-4) #NOTE: Weight decay to improve generalization and combat overfitting, but the optimal value may require tuning. A common default is 1e-4 for AdamW.

    # Initialize mixed precision, multi-GPU training and gradient accumulation with accelerator
    model, optimizer, train_dataloader, valid_dataloader = accelerator.prepare(model,
                                                                               optimizer,
                                                                               train_dataloader,
                                                                               valid_dataloader)
