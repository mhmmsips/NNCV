...
# NOTE: Normalize is intentionally absent here; it runs after weather aug, same as train.py
...
def apply_full_pipeline_train(img_tensor: torch.Tensor,
    ...
    Pipeline: horizontal flip → weather aug → normalize → de-normalize for saving
    ...
    # Apply weather aug on the pre-normalization uint8 image
    img_numpy = tensor_to_uint8_numpy(img_tensor)
    img_numpy = weather_aug(image=img_numpy)["image"]
