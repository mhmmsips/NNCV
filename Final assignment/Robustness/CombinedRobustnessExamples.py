"""
Visualise the combined FDA + Weather Augmentation pipeline on Cityscapes images.

Augmentation order: FDA --> WA --> ColorJitter --> GaussianBlur --> flip --> normalize.
...
Four export directories are created inside the Robustness folder:
  CombinedTrainingExamples; 50 training images, FDA + WA at 100%
  CombinedValidationExamples; the 2 W&B validation images, each augmented 25 times, FDA + WA at 100%
  FullyAugmentedCombinedTrainingExamples; 50 training images, full pipeline (FDA + WA + jitter/blur/flip)
  FullyAugmentedCombinedValidationExamples; the 2 W&B validation images, each augmented 25 times, full pipeline
...
dir_combined_train = os.path.join(script_dir, "CombinedTrainingExamples")
dir_combined_val = os.path.join(script_dir, "CombinedValidationExamples")
dir_full_combined_train = os.path.join(script_dir, "FullyAugmentedCombinedTrainingExamples")
dir_full_combined_val = os.path.join(script_dir, "FullyAugmentedCombinedValidationExamples")

for d in [dir_combined_train, dir_combined_val, dir_full_combined_train, dir_full_combined_val]:
...
# FDA transform applied to 100% of images; beta_limit=(0.0, 0.1): the paper shows beta <= 0.01 produces clean style transfer without visible artefacts.
...
fda_aug = A.Compose([A.FDA(beta_limit=(0.0, 0.1), p=1.0)])
...
def apply_combined_only(img_tensor: torch.Tensor,
                        weather_aug) -> torch.Tensor:
    """Apply FDA followed by WA and return a uint8 CHW tensor ready for PIL saving.
...
def apply_full_pipeline_combined(img_tensor: torch.Tensor,
                                  mask_tensor: torch.Tensor,
                                  weather_aug) -> torch.Tensor:
    """Apply the full "both" training pipeline from train.py and return a uint8 CHW tensor.

    Pipeline: FDA --> WA --> ColorJitter --> GaussianBlur --> flip --> normalize --> de-normalize for saving
    ...
    # Photometric augmentations after the domain augmentations, then the joint flip
...
def load_train_dataset() -> Cityscapes:
    ...
    NOTE: Normalize is excluded from train_img_transform (matches train.py); it runs after all augmentations.
    """
...
# %%
# Export 1: CombinedTrainingExamples
# 50 random training images, each augmented once with FDA + WA at 100% probability

print("Exporting CombinedTrainingExamples...")
...
for export_idx, ds_idx in enumerate(train_indices):
    ...
    img_augmented = apply_combined_only(img, weather_aug_always)
...
# %%
# Export 3: FullyAugmentedCombinedTrainingExamples
# Same 50 training images, full pipeline (FDA + 50% WA + jitter/blur/flip)

print("Exporting FullyAugmentedCombinedTrainingExamples...")

for export_idx, ds_idx in enumerate(train_indices):
    img, mask = train_ds[ds_idx]

    img_augmented = apply_full_pipeline_combined(img, mask, weather_aug_50pct)
...
# %%
# Export 4: FullyAugmentedCombinedValidationExamples
# Same 2 W&B validation images, each augmented 25 times with the full combined pipeline

print("Exporting FullyAugmentedCombinedValidationExamples...")
